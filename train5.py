...
parser.add_argument('--outf', default='myImgs5/', help='folder to output images and model checkpoints')
...
netG = model5.Generator().to(device)
netD = NetD(opt.ndf).to(device)

criterion = nn.BCELoss()
optimizerG = torch.optim.Adam(netG.parameters(), lr=0.005, betas=(opt.beta1, 0.999))
optimizerD = torch.optim.Adam(netD.parameters(), lr=0.0007, betas=(opt.beta1, 0.999))

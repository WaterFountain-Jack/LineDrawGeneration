...
parser.add_argument('--epoch', type=int, default=150, help='number of epochs to train for')#训练的轮数
...
for epoch in range(1, opt.epoch + 1):
    for i, (img) in enumerate(zip(dataloader,dataloader2)):
        ...
        #plt.imshow(torch.squeeze(fake.data[0].cpu()).numpy().reshape(opt.imageSize, opt.imageSize, 3))
        # 让D尽可能把G生成的假图判别为1
        label.data.fill_(real_label)
        label = label.to(device)
        output = netD(fake)

...
'''
    在model6的基础上进行尝试，model6的gen比较混乱，并且可能包含原图的信息太多了
    
    model7在训练150 轮的时候 （学习率都为0.0005）发现判别器已经全为0了  说明是生成器的精度不够？
'''
# 矩阵相乘
...
'''
    之前有drop_rate = 0.1  nn.Dropout(p=0.5)
'''
class Generator(nn.Module):
    def __init__(self,num_heads=4,mlp_ratio=4.,qkv_bias=False, qk_scale=None, drop_rate=0, attn_drop_rate=0.,):
        ...
        self.pos_embed_5 = nn.Parameter(torch.zeros(1, 4096, 8))
        self.pos_embed_6 = nn.Parameter(torch.zeros(1, 16384, 8))
    ...
    def forward(self, x):
        ...
        # [-1,1024,32] -> [-1,4096,8]
        y, H, W = pixel_upsample(y, H, W)
        y =  y + self.pos_embed[5]
        #block里面是(64*64*8)^2 需要的内存太大了，这时候就需要用到分块的思想，计算每一块之间的注意力  之前是y = self.blocks[5](y)
        B, _, C = y.size()
        y = y.view(B, H, W, C)
        ...
        output = self.tRGB(y.permute(0, 2, 1).view(B, 8, H, W))
